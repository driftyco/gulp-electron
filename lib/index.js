var Decompress, File, PLUGIN_NAME, PluginError, ProgressBar, Promise, asar, asarPackaging, async, chalk, childProcess, distributeApp, distributeBase, download, electron, fs, getApmPath, grs, isDir, isExists, isFile, mv, mvAsync, packaging, path, plist, rebuild, rm, rmAsync, signDarwin, spawn, through, unzip, util, winresourcer, wrench;
    ...
    return Promise.map(options.platforms, function(platform) {
      ...
      return new Promise(function(resolve, reject) {
        return Promise.resolve().then(function() {
          ...
          return distributeBase(platformPath, cacheedPath, copyOption, electronFilePath, targetAppPath);
        }).then(function() {
          if (!options.rebuild) {
          ...
          if (platform.indexOf('darwin') >= 0) {
            ...
            return util.log('Finished updating plist file');
          }
        }).then(function() {
          var atomIcnsPath, readStream;
          if (options.icnsPath && platform.indexOf('darwin') >= 0) {
            ...
            return readStream.pipe(fs.createWriteStream(atomIcnsPath));
          } else if (platform.indexOf('win') >= 0 && options.iconPath) {
...
distributeBase = function(platformPath, cacheedPath, copyOption, electronFilePath, targetAppPath) {
